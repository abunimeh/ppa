...
class PhysicalRpt:
    ...
    def match_line(regex1, regex2, regex3, file_line):
        import re
        words_to_match = r'(%s[\s]*%s[\s]*%s[\s]*.*):+[\s]*([\d]+[\.]*[\d]*.*)' % (regex1, regex2, regex3)
    ...
    def search_file(file):
        ...
        for file_line in file_lines:
            ...
            if found_util:
                rpt_data.found_util[1] = FormatMetrics.format_metric_values(found_util.group(2))
            elif found_short:
                rpt_data.found_short = PhysicalRpt.replace_space("apr Shorts"), FormatMetrics.format_metric_values(found_short.group(2))
                metric_list.append(rpt_data.found_short)
            elif found_total_error:
                rpt_data.found_total_error[1] = FormatMetrics.format_metric_values(found_total_error.group(2))
            elif found_total_mem:
                rpt_data.found_total_mem[1] = FormatMetrics.format_metric_values(found_total_mem.group(2))
