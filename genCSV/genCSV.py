...
class GenerateMetrics:
    ...
    def determine_testcases(command_line_args):
        import json
        import os
        config_file = os.path.join(os.path.dirname(command_line_args[0]), 'config.json')
        ...
        # 4 is for searching by the date folder and three levels into that
            ...
            print("Found %s testcases" % len(test_cases_list))
            print("Testcases:", test_cases_list)
        GenerateMetrics.get_metrics(test_cases_list, config_file)

    @staticmethod
    def get_metrics(test_cases_list, config_file):
        ...
        for test_case in test_cases_list:
            ...
            list_of_files = findFiles.search_dir(test_case, tool, config_file)
            if tool is "cadence":
                temp_metric_collections = GetCadenceMetrics.get_cadence_metrics(list_of_files, test_case, tool, config_file)
            else:
                temp_metric_collections = GetSynopsysMetrics.get_synopsys_metrics(list_of_files, test_case, tool, config_file)
