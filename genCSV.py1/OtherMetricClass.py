            ...
            for line in lines:
                for metric_object, metric_object_name in list_of_files[ending].items():
                    if metric_object_name["found_section_reg_exp"]["begin_line"] == "none":
                        ...
                        met_value = OtherMetricClass.match_line(metric_object_name["metric_reg_exp"], line)
                        if met_value:
                        ...
                        if last_line != line:
                            beginning_line = OtherMetricClass.match_line(metric_object_name["found_section_reg_exp"]["begin_line"], line)
                            if beginning_line:
                                if last_line != line:
                                    section = 1
                                continue
                            ending_line = OtherMetricClass.match_line(metric_object_name["found_section_reg_exp"]["end_line"], line)
                        if section == 1:
                            if metric_object_name["stage"] == "none":
                                met_name = metric_object_name["metric_name"]
                            elif metric_object_name["stage"] == 1:
